...
## Zip-it (Add to file.extX or file.zip)
...
```
`Usage: zip-it <zipfile.ext> <entry> <file to add>`  
...
Both above problem tasks could be best handled with 7-Zip commands but often we want a simple one task command line helper or a more GUI approach so I wrote a simple "SendTo" command to check files are extractable or save a list of contents which too could be a single line command. But as a GUI it is much larger than a simple one line so can be found as ListArc at  https://github.com/GitHubRulesOK/MyNotes/tree/master/C%23
...
# HTML
**NOTE current msedge version 141 is again having problems related to user folder** so you may need a temporary user folder as shown in first example
...
This is the enhanced single line minified version of a simple version editor for https://superuser.com/a/1933379/1769247
Improvements are in errorlevel reporting and batch progress report. Each attempt should output:
Status; message; filename and the errorlevel is either 0, 1 or 2.
```
